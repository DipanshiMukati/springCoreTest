...
public class TradeCtl extends BaseCtl {
	...
	protected boolean validate(HttpServletRequest request) {
		...
		else if ( 5 >request.getParameter("userId").length()) {
			request.setAttribute("userId", "userId  must contains 5 character");
			pass = false;
		}  
		
		else if (!DataValidator.isAlphanumeric(request.getParameter("userId"))) {
		request.setAttribute("userId", "userId  must contains Alphanumeric Character only");
		pass = false;
	} 
